...
size_t findShortestPath(std::vector<const KeypadMap*> kms, std::string keypad_chars) {
    if (kms.empty()) {
    ...
    for (char c : keypad_chars) {
        ...
        size_t shortest_subpath_size = -1;
...
int main() {
    ...
    for (const auto& code : codes) {
        const auto shortest_path_size = findShortestPath({&DIR_KEYPAD_MAP, &DIR_KEYPAD_MAP, &NUM_KEYPAD_MAP}, code);
        auto complexity = utils::parse<size_t>(code) * shortest_path_size;
        std::cout << code << " " << utils::parse<size_t>(code) << " " << shortest_path_size << std::endl;
