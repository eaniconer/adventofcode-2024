...
int main() {
    ...
    auto ei0 = convertRCLtoVertexIndex(er, rc, 0);
    auto ei1 = convertRCLtoVertexIndex(er, rc, 1);

    std::unordered_set<std::pair<size_t, size_t>> positions;
    positions.insert(end);

    std::unordered_set<size_t> to_see;
    if (d[ei0] == best_dist)
        to_see.insert(ei0);
    if (d[ei1] == best_dist)
        to_see.insert(ei1);
    ...
    while (!to_see.empty()) {
        ...
        for (auto v : to_see) {
            for (auto [u, w] : graph[v]) {
                if (d[u] + w == d[v]) {
                    new_to_see.insert(u);
                    positions.insert(convertVertexIndexToRC(u));
                }
            }
        }
        std::swap(new_to_see, to_see);
    }
    std::cout << "Tiles count: " << positions.size() << std::endl;
